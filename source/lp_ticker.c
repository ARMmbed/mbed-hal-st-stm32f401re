...
void lp_ticker_init(void) {
	...
	} else if((uint32_t)(SystemCoreClock / 2000) <= 0xFFFFU) {
 		/* betzw: having a 16-bit prescaler, beyond settings work only for system clock 
		          frequencies (i.e. HCLK) below 131.072.000Hz!!! */
		TimMasterHandle.Init.Prescaler = (uint32_t)(SystemCoreClock / 2000) - 1; // 0.5 ms tick
       } else {
